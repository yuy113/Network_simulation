...
#simulation scenario 1 in the paper
#under BA random graph model
#Change the indices of four clusters where nodes with positive scores and edges with positive scores
...
randba.pos.network.new1<-function(network.size,m,power,n.size){
  ...
  n.start.cluster3<-vcount(g)-6*n.size+1
  
  n.start.cluster4<-vcount(g)-n.size+1
  
  cluster1.id<-1:n.size
  cluster2.id<-seq(n.start.cluster2,n.start.cluster2+n.size-1,1)
  cluster3.id<-seq((vcount(g)-6*n.size+1),(vcount(g)-5*n.size),1)
  ...
  library(MASS)
  ind.cluster1<-unlist(apply(g.edge.id,1,function(x) return(length(intersect(x,cluster1.id))==2)))
  ...
  #n.start.cluster4<-floor(vcount(g)/2)
  
  ind.cluster4<-unlist(apply(g.edge.id,1,function(x) return(length(intersect(x,cluster4.id))==2)))
  ...
  ind.cluster3<-unlist(apply(g.edge.id,1,function(x) return(length(intersect(x,cluster3.id))==2)))
  if(sum(ind.cluster3)>=1){
    ...
    score.edge3<-runif(n.edge3,min=2,max=4)
  ...
  ind.cluster2<-unlist(apply(g.edge.id,1,function(x) return(length(intersect(x,cluster2.id))==2)))
  if(sum(ind.cluster2)>=1){
    ...
    score.edge2<-runif(n.edge2,min=0.1,max=2)
